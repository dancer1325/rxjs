...
* == Subjects' variant /
  * 👀send old values -- to -- NEW subscribers👀
    * == `BehaviorSubject`
  * ⚠️record a part of the Observable execution ⚠️& replays them -- to -- NEW subscribers
* constructor
  * `new ReplaySubject(bufferSize: number, windowTime?: number)`
    * `bufferSize`
      * NUMBER of LAST record values -- to -- store
    * `windowTime`
      * == how old record values can be
